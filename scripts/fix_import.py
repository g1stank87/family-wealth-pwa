...
import_start = content.find("importFromExcel(event)")
import_end = content.find("// ========== F032:", import_start)

new_import = '''importFromExcel(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Section column structure by subsection:
        // Type1 (自用房产/投资性x房): E=万元/㎡, F=㎡, G=E*F
        // Type2 (自用车辆/自用其他): E=买入总价(万元), F=0
        // Type3 (收藏品/REITs/股票/债券/基金): E=元/份(or股), F=份(or股), G=E*F/10000
        // Type4 (股权): E=估值万元, F=股比, G=E*F
...
if import_start < 0:
    print("ERROR: importFromExcel not found!")
else:
    content = content[:import_start] + new_import + "\n\n" + content[import_end:]
